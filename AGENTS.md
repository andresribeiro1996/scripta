...
- Concurrent agents: never edit directly in the primary checkout (`~/Documents/scripta` itself, whatever branch it happens to be on) — treat it as what new worktrees branch off, not a workspace. Every agent session starts with `EnterWorktree` (or `git worktree add .claude/worktrees/<name> -b <branch> origin/main` where that tool isn't available), then runs `npm run dev:link-deps` once inside it — it symlinks `node_modules` from the primary checkout instead of paying for a full reinstall, and falls back to a real `npm install` on its own if this branch's `package-lock.json` actually differs. Two sessions in the same checkout share one git index; a `git add`/`commit` there can silently sweep up the other session's staged files.
