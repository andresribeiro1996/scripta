...
Each workspace's `node_modules` (`frontend/`, `mobile/`, …) becomes one symlink to the primary's. The root `node_modules` can't: it holds npm's workspace links (`@scripta/shared -> ../../packages/shared`), and a relative link followed through a symlink into the primary resolves to the primary's source, so typecheck, tests and Metro would silently compile against main's `packages/shared`. Instead the root becomes a real directory whose entries each symlink into the primary's, except the workspace packages, which link to this worktree's own copies. Changes to `packages/shared` are picked up after `npm run build --workspace packages/shared`, as in the primary. A worktree linked before this change (root `node_modules` is itself a symlink) is converted by re-running `npm run dev:link-deps`.
