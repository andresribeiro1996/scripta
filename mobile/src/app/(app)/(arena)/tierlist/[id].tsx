...
export default function TierlistEditorRoute() {
  ...
  return <TierlistEditorScreen tierlist={tierlist} startInRank={rank === "1"} onUpdated={(updated) => client.setQueryData<Tierlist[]>(["tierlists"], (items = []) => items.map((item) => item.id === updated.id ? updated : item))} />;
}
