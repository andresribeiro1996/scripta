    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      ...
      <Sheet visible={selected !== null && picking === null} title="Block settings" onClose={() => setSelectedId(null)}>
        {selected ? <ScrollView contentContainerStyle={styles.sheet}>
          ...
          {selected.type === "stats" ? <View style={styles.sheet}>{ALL_STAT_METRICS.map((metric) => <Pressable key={metric} onPress={() => updateSelected((block) => block.type === "stats" ? { ...block, metrics: block.metrics.includes(metric) ? block.metrics.filter((item) => item !== metric) : [...block.metrics, metric] } : block)}><Text style={[typography.body, { color: selected.metrics.includes(metric) ? colors.accent : colors.text }]}>✓ {metric}</Text></Pressable>)}</View> : null}
          ...
          <View style={styles.row}><Button label="←" variant="secondary" onPress={() => setBlocks(changeBlockLayout(currentBlocks, selected.id, { x: selected.layout.x - 1 }))} /><Button label="→" variant="secondary" onPress={() => setBlocks(changeBlockLayout(currentBlocks, selected.id, { x: selected.layout.x + 1 }))} /><Button label="↑" variant="secondary" onPress={() => setBlocks(changeBlockLayout(currentBlocks, selected.id, { y: selected.layout.y - 1 }))} /><Button label="↓" variant="secondary" onPress={() => setBlocks(changeBlockLayout(currentBlocks, selected.id, { y: selected.layout.y + 1 }))} /></View>
      ...
      <Sheet visible={picking !== null} title={`Choose ${picking ?? "content"}`} onClose={() => setPicking(null)}>
        <ScrollView contentContainerStyle={styles.sheet}>
          ...
          {picking === "image" ? (gallery.data ?? []).map((image) => <Pressable key={image.id} onPress={() => { updateSelected((block) => block.type === "image" ? { ...block, imageId: image.id } : block); setPicking(null); }}><Text style={[typography.body, { color: colors.text }]}>{image.filename}</Text></Pressable>) : null}
