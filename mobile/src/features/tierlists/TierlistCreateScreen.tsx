  ...
  return <Screen top={false} style={styles.screen}>
    ...
    {step === 1 ? <ScrollView contentContainerStyle={styles.tiers} keyboardShouldPersistTaps="handled">
      ...
      {tiers.map((tier, index) => <View key={tier.id} style={styles.tier}>
        ...
        <View style={styles.grow}><Input label={`Tier ${index + 1}`} value={tier.label} onChangeText={(label) => setTiers((value) => value.map((entry) => entry.id === tier.id ? { ...entry, label } : entry))} maxLength={30} /></View>
        <Button label="Remove" variant="secondary" disabled={tiers.length === 1} onPress={() => setTiers((value) => value.filter((entry) => entry.id !== tier.id))} />
      </View>)}
...
const styles = StyleSheet.create({
  ...
  swatch: { width: 20, height: 44, borderRadius: radii.sm },
