...
- Disconnecting a platform deletes AtMyShelf's own copy of the token; it does not separately call that platform's revoke endpoint to invalidate the token on their end. Most of these tokens are short-lived by platform design anyway, and a user can always revoke from that platform's own connected-apps settings — but a real revoke-on-disconnect call (where each platform's API supports one) would be a natural small follow-up, not implemented here.
...
- `npm run build`/`npm start` now work: `tsc` still only compiles `.ts`, so `scripts/copy-assets.mjs` runs after it and mirrors every `schema.sql` and `auth/public/console.html` into `dist/` at the same relative path (these are read from disk at runtime, relative to the reading file's own location). Verified by booting the compiled output and hitting `/health`. If a module starts reading some other file type at runtime, add its glob to that script's `PATTERNS` — it exits non-zero if it matches nothing, so a build that would produce an unbootable `dist/` fails rather than shipping.
