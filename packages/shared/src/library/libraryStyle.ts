...
// frontend's components/LibraryCanvas.tsx. Lives on the library document
// (`data.style`, see frontend's api/library.ts) rather than localStorage:
// it's a per-account preference the user set deliberately, same
// reasoning as the library `name` (groups.ts's Group and that field both
// went through the same "should this be server-side" call) — it should
// follow the account across browsers/devices, not reset on a new
// machine. Never touched by an importer, so it survives merges the same
// way `name` and `groups` do (see merge.ts's mergeLibraryData).
//
// This module holds the data half only — types, defaults, ranges, and
// the pure resolve/priority functions. The CSS serializers
// (cardFontFamilyCss, blockFontFamilyCss, resolveBorderColor) are
// deliberately excluded: they return comma-joined font stacks and
// var()/color-mix() strings that only mean something to a CSS renderer,
// so they stay in frontend/src/lib/libraryStyle.ts. Task 4D builds the
// React Native equivalents (a single font family, no var()/color-mix()).
 ...
 *  not shared with mural blocks' `BlockFontFamily` (this file's
...
export const CARD_FONT_FAMILY_OPTIONS: Array<{ value: CardFontFamily; label: string; css: string }> = [
  ...
  // Self-hosted (see frontend's index.css) — real, distinctive typefaces
  // rather than another OS-default look-alike.
...
export interface LibraryStyleSettings {
   ...
   *  — see frontend's BookCard.tsx `resolveBorderColor()`. */
   ...
   *  frontend's BookCard.tsx `showOverlayText`). */
   ...
   *  all, defeating the point of a library view. See frontend's
   *  BookCard.tsx `hasCover` state for where that's enforced. */
   ...
   *  sized in `em` relative to this (see frontend's BookCard.tsx), not
   *  Tailwind's rem-based text-* utilities, which measure against the
   *  document root and wouldn't respond to this at all. */
  cardFontSize: number;
   ...
   *  actually downloaded per self-hosted family (see frontend's
   *  index.css) — this renders via the browser's own synthesized bold
   *  rather than a second font file, same as `cardItalic` uses
   *  synthesized oblique. */
  cardBold: boolean;
  ...
  // application shell — see frontend's components/LibraryCanvas.tsx for
  // where that line is drawn and why it moved there.
...
// frontend's scripts/test-library-style.mts asserts for every numeric
// default.
...
// --- Per-series style overrides (see groups.ts's Group.style) ---
//
 ...
 *  — see `book._style`, frontend's BookCard.tsx "Style" button). Layout/
 *  page fields always come from `libraryStyle` regardless of either
 *  override, since neither can contain them (see `PerCardStyle`'s type).
 *
...
// --- Mural block style (see murals.ts's MuralBlock.style, frontend/Task
// 3C) ---
//
// Every mural block (frontend's components/murals/BlockStylePanel.tsx,
// opened from its own "Style" button on MuralCanvas.tsx — a separate
// control from the gear icon's content config, same Style-vs-Cover split
// BookCard.tsx already has) can have its own appearance — background,
// radius, border, shadow, hover, font, size, text color.
//
...
export const BLOCK_FONT_FAMILY_OPTIONS: Array<{ value: BlockFontFamily; label: string; css: string }> = [
  ...
  { value: "serif", label: "Serif (system)", css: "ui-serif, Georgia, Cambria, 'Times New Roman', Times, serif" },
  { value: "mono", label: "Monospace (system)", css: "ui-monospace, SFMono-Regular, Menlo, Consolas, 'Liberation Mono', monospace" },
  // Self-hosted (see frontend's index.css) — same three as CardFontFamily's
  // own additions, duplicated rather than shared, per this file's
  // isolation rule (see BlockStyle's own comment below).
  { value: "playfairDisplay", label: "Playfair Display", css: "'Playfair Display', ui-serif, Georgia, serif" },
  { value: "inter", label: "Inter", css: "'Inter', ui-sans-serif, system-ui, sans-serif" },
  { value: "jetbrainsMono", label: "JetBrains Mono", css: "'JetBrains Mono', ui-monospace, SFMono-Regular, monospace" }
];
 ...
 *  (frontend's StyleControls.tsx) works for a block's style unmodified
 *  via ordinary structural typing — that's a UI-component-reuse
 *  convenience, not a type relationship; nothing here imports or
...
export type BlockStyle = {
   ...
   *  family (see frontend's index.css), same reasoning as PerCardStyle's
   *  `cardBold`. */
  bold: boolean;
 ...
 *  container queries (see frontend's index.css `.book-card` rules and
 ...
 *
 *  The phone rule (frontend's index.css, behind a max-width query) scales
 *  the column floor so that the DEFAULT card size lands on exactly this
 *  many columns, and every other setting scales in proportion. It is
 ...
 *  else does. Kept in sync by hand with the `@media` query in frontend's
 *  index.css; CSS can't read a TS constant, and the alternative (a
 *  <style> tag rendered per grid) would spend a style element on a
 *  number that never varies at runtime. */
