// Book search for the Add Book flow (frontend's components/
// AddBookModal.tsx) and the mapping from a chosen result to a library
// book record.
//
// The mirror image of bookMetadata.ts, which pins down ONE already-known
// book (exact title+author match) to show its summary/rating on the
// detail sheet. Here we need the opposite: a free-text "ISBN or title or
...
export function mapOpenLibraryDoc(doc: Record<string, unknown>): BookSearchResult | null {
  ...
  // normalizeIsbn-based book matching (merge.ts) and the backend
...
export interface ManualBookFields {
  ...
  isbn: string;
  publisher: string | null;
  /** Same 0/1/2 semantics as every other book (see goodreads.ts). */
  readStatus: number;
  rating: number | null;
 ...
 *  (goodreads.ts's is the closest sibling — no cover key, ISBN+
