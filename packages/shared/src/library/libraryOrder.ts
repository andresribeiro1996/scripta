// Display order for the main Library grid (frontend's LibraryPage.tsx) —
// NOT for the underlying `books` array in storage, which stays in plain
// merge/import order (see merge.ts); this is purely a render-time
// derivation.
//
// Rules:
...
//      organizational tool (see frontend's GroupsPage.tsx), not a layout
//      one, so a book being in a collection has zero effect on where it
//      renders here.
...
import { orderedGroupBooks, type Group } from "./groups.js";
 ...
 *  after every merge/first-import (see frontend's LibraryPage.tsx's
 *  handleFileChosen), before saving. New books are appended after the
 *  current maximum, in their incoming order; a book that already has one
 *  keeps it untouched (including through a merge — see mergeBookPair in
 *  merge.ts, fixed to preserve app-managed fields like this one across a
 *  re-import). */
 ...
 *  priority (see libraryStyle.ts's effectiveCardStyle) — built from the
 *  exact same units orderLibraryBooks() renders, so "which series is
