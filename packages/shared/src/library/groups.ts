...
export interface Group {
  ...
  type: GroupType;
  name: string;
  /** References into books via bookKey() (see merge.ts) rather than
   ...
   *  for every book in it (see libraryStyle.ts's effectiveCardStyle).
 ...
 *  (frontend's PerCardStylePanel) is responsible for seeding a full
 *  `PerCardStyle` when turning customization on. */
 ...
 *  (see merge.ts's bookKey and the delete handlers in frontend's
