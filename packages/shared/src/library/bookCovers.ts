// Assigning a gallery image (see frontend's api/gallery.ts) as a book's
// cover — pure logic only, same reasoning as groups.ts: the library
// document is an opaque blob as far as the backend's `library` module is
// concerned, so "which gallery image is this book's cover" lives entirely
// as fields on the book object, no backend change needed beyond the
// gallery module itself.
//
...
// components/BookCard.tsx's CoverImage, and merge.ts's "keep whichever
// version has a cover" rule) — assigning a gallery image just sets that
// same field. `_coverImageId` is the one genuinely new field: bookkeeping
// so a later "delete this gallery image" (scrubImageFromBooks) knows
 ...
 *  GET /covers/resolve, see frontend's api/covers.ts) by clearing both
 *  fields — CoverImage falls straight back to that once `_coverUrl` is
 *  gone. */
 ...
 *  no-op-when-untouched convention as groups.ts's
